...
public class MarkerJSONParser {
	...
	public List<HashMap<String,String>> parse(JSONObject jObject){		
		...
		try {/** Retrieves all the elements in the 'markers' array */
	...
	private List<HashMap<String, String>> getMarkers(JSONArray jMarkers){
		...
		for(int i=0; i<markersCount;i++){
			try{/** Call getMarker with marker JSON object to parse the marker */
	...
	private HashMap<String, String> getMarker(JSONObject jMarker){
		...
		String lat = "-NA-";
		String lng ="-NA-";
		try{// Extracting latitude, if available
			if(!jMarker.isNull("lat")){lat = jMarker.getString("lat");}
			// Extracting longitude, if available
			if(!jMarker.isNull("lng")){lng = jMarker.getString("lng");}					
			marker.put("lat", lat);
			marker.put("lng", lng);}
		catch (JSONException e){e.printStackTrace();}		
		return marker;
